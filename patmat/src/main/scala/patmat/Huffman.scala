...
object Huffman {
    ...
    def times(chars: List[Char]): List[(Char, Int)] = {
      def timesAcc(chars: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = 
        if(chars.isEmpty) acc
          ...
          val letterInstances = chars.count { _ == chars.head }          
          val nextAcc = acc ::: List((chars.head, letterInstances))
          val nextChars = chars.filterNot { _ == chars.head }
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      def orderedLeavesAcc(freqs: List[(Char, Int)], acc: List[Leaf]): List[Leaf] = {
        if(freqs.isEmpty) acc
        else {
          val maxItem = freqs.maxBy(_._2)
          val leaf = new Leaf(maxItem._1, maxItem._2)
          val nextAcc = List(leaf) ::: acc
          
          val geeks = freqs.filterNot(_._1 == maxItem._1)
          orderedLeavesAcc(geeks, nextAcc)
        }
      }
      orderedLeavesAcc(freqs, Nil)
    }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = {
      if(singleton(trees)) trees 
        ...
        val split = tail.span { weight(_) < fork.weight }
        split._1 ::: List(fork) ::: split._2
      }
    }
                          ...
                          Fork(
                              Fork(
                                  Fork(
                                      ...
                                      Fork(
                                          Leaf('k',745),
                                          Leaf('w',1747),List('k','w'),2492),List('z','k','w'),4585),
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def stupidGetCodeForChar(subTree: CodeTree, char: Char, path: List[Bit]): List[Bit] = {
        subTree match {
          case f: Fork => {
            val leftResult = stupidGetCodeForChar(f.left, char, path ::: List(0))
            if(!leftResult.isEmpty) leftResult
            else stupidGetCodeForChar(f.right, char, path ::: List(1))
          }
          case l: Leaf => {
            if(l.char == char) path
            else Nil
      ...
      def encodeAcc(tree: CodeTree, text: List[Char], acc: List[Bit]): List[Bit] = {
        if(text.isEmpty) acc
          ...
          val code = stupidGetCodeForChar(tree, text.head, Nil)
          encodeAcc(tree, text.tail, acc ::: code)
        }
      }
   ...
   */
    def convert(tree: CodeTree): CodeTable = {
      def convertAcc(subTree: CodeTree, path: List[Bit]): CodeTable = {
        subTree match {
          case f: Fork => convertAcc(f.left, path ::: List(0)) ::: convertAcc(f.right, path ::: List(1))
          case l: Leaf => List((l.char, path))
        }
      }
      convertAcc(tree, Nil)
    }
   ...
   */
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def encodeAcc(table: CodeTable, text: List[Char], acc: List[Bit]): List[Bit] = {
        if(text.isEmpty) acc
        else {
          val code = table.find(_._1 == text.head).head._2
          encodeAcc(table, text.tail, acc ::: code)
        }
      }
